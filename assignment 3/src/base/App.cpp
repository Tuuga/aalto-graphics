...
void App::initRendering()
{
	...
	glVertexAttribIPointer(ATTRIB_JOINTS1, 3, GL_INT, sizeof(WeightedVertex), (GLvoid*) offsetof(WeightedVertex, joints[0]));
	glEnableVertexAttribArray(ATTRIB_JOINTS2);
	glVertexAttribIPointer(ATTRIB_JOINTS2, 3, GL_INT, sizeof(WeightedVertex), (GLvoid*) offsetof(WeightedVertex, joints[3]));
	glEnableVertexAttribArray(ATTRIB_WEIGHTS1);
	glVertexAttribPointer(ATTRIB_WEIGHTS1, 3, GL_FLOAT, GL_FALSE, sizeof(WeightedVertex), (GLvoid*) offsetof(WeightedVertex, weights[0]));
	glEnableVertexAttribArray(ATTRIB_WEIGHTS2);
	glVertexAttribPointer(ATTRIB_WEIGHTS2, 3, GL_FLOAT, GL_FALSE, sizeof(WeightedVertex), (GLvoid*) offsetof(WeightedVertex, weights[3]));
	...
	auto ssd_prog = new GLContext::Program(
		...
		layout(location = 3) in ivec3 aJoints1;
		layout(location = 4) in ivec3 aJoints2;
		layout(location = 5) in vec3 aWeights1;
		layout(location = 6) in vec3 aWeights2;
